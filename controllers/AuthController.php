...
class AuthController extends Controller
{
    ...
    public function refresh(Request $request)
    {
        if (DotEnv::get('API_AUTH_METHOD') != 'JWT') {
            $this->response(['message' => 'Invalid auth method'], Response::UNAUTHORIZED);
        }
        ...
        if (!$user) {
            $this->response(['message' => 'User does not exist'], Response::UNAUTHORIZED);
        }
        if (!hash_equals($user->password, hash('sha256', $password))) {
            $this->response(['message' => 'Invalid credentials'], Response::UNAUTHORIZED);
        }
        ...
        try {
            ...
            $this->response(['message' => $e->getMessage()], Response::INTERNAL_SERVER_ERROR);
        }
    ...
    public function access()
    {
        ...
        if (!Server::authorization()) {
            $this->response(['message' => 'Refresh-token not provided'], Response::BAD_REQUEST);
        }

        $token = explode(' ', Server::authorization())[1];
        $sysApiTokens = new SysApiTokens();
        if (!$sysApiTokens->exists($token)) {
            $this->response(['message' => 'Token is invalid'], Response::UNAUTHORIZED);
        }
        if ($sysApiTokens->isRevoked($token)) {
            $this->response(['message' => 'Token revoked'], Response::UNAUTHORIZED);
        }
